...
class TCGenerator:
    ...
    def tcBlock(self):
        ...
        hour = 0
        l_val = 0
        for val in itertools.islice(self.generateFPS(),
                                    int(self.frameRate*self.duration/1000)):
                ...
                if sec == 60:
                    ...
                    if minute == 60:
                        ...
                        if hour == 24:
                            hour = 0
            l_val = val
            block.append(self.tcRaw(val, sec, minute, hour).tolist())
    ...
    def audioData(self, duration=1000):  # 18.375 bytes/samples per bit --> 1=18samples, 0=19samples
        ...
        tc_audio = (tc_audio * ((2**15)-1)).astype(np.int16)
